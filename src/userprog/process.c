...
process_execute (const char *file_name) 
{
  ...
  if(pcb->pid >= 0) {
    list_push_back (&(thread_current()->c_thread),&(pcb->elem));
  }
...
start_process (void *pcb_)
{
  ...
  if(args->argv == NULL){
	printf("[Error] Kernel Eroor: Not enough memory\n");
	goto finish_step;
  }

  for (curr = strtok_r(fn_copy, " ", &leftover); curr != NULL;curr = strtok_r(NULL, " ", &leftover))
  {
...
/* Waits for thread TID to die and returns its exit status.  If
   ...
   does nothing. */
int
process_wait (tid_t child_tid) /*UNUSED eliminated*/ 
{
  struct thread *t = thread_current ();
  struct list *child_list = &(t->c_thread);
...
process_exit (void)
{
  ...
  if (p_d != NULL) 
	...
	sema_up(&(cur->memory_preserve));	/*updated*/
	sema_down(&(cur->c_thread_lock));	/*updated*/
}
...
static void push_arguments (struct arguments* args, void **esp)
{
  ...
  int i, len = 0;
  void* argv_addr[args->argc];
  for (i = 0; i < args->argc; i++) {
    len = strlen(args->argv[i]) + 1;
    *esp -= len;
    memcpy(*esp, args->argv[i], len);
    argv_addr[i] = *esp;
  }
