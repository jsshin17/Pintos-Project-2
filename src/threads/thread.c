...
thread_unblock (struct thread *t) 
{
  ...
  list_insert_ordered (&ready_list, &t->elem,(list_less_func *) &thread_compare_priority,0);
...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread) 
    list_insert_ordered (&ready_list, &cur->elem,(list_less_func *) &thread_compare_priority,0);
...
{
  return CONVERT_TO_INT_NEAR (MULT_MIXED(load_avg,100));
}
int
thread_get_recent_cpu (void) 
{
  struct thread *t = thread_current();
  return CONVERT_TO_INT_NEAR(MULT_MIXED(t->recent_cpu,100));
}
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  list_push_back (&all_list, &t->allelem);
		...
		sema_init(&t->c_exe, 0);
		sema_init(&t->c_thread_lock, 0);
		sema_init(&t->memory_preserve, 0);
		list_init(&(t->c_thread));
		list_push_back(&(running_thread()->c_thread), &(t->c_thread_elem));
		t->flag = 0;
		t->already_wait = 0;
	#endif
}
...
void mlfqs_calc_priority(struct thread *t, void *aux)
{
	if(t!=idle_thread){
		t->priority = PRI_MAX - CONVERT_TO_INT_NEAR(DIV_MIXED(t->recent_cpu,4))- t->nice*2;
		if(t->priority < PRI_MIN) t->priority = PRI_MIN;
		else if(t->priority>PRI_MAX) t->priority=PRI_MAX;
	}
}
void mlfqs_calc_recent_cpu(struct thread *t, void *aux)
{
	if(t!=idle_thread){
		int load = MULT_MIXED(load_avg,2);
		int coef = DIV_FP(load,ADD_MIXED(load,1));
...
{
	struct thread *t = thread_current();
	if(t!=idle_thread) t->recent_cpu =ADD_MIXED(t->recent_cpu,1);
}
